...
KapyHolograms exposes a clean Java API for other plugins to create and manage holograms programmatically.
...
Hologram hologram = api.hologramBuilder()
    ...
    .renderDistance(32.0)
    .line(HologramLine.text("<gold>Hello, world!"))
    .line(HologramLine.text("<gray>Welcome, <white>%player_name%"))
...
page2.addLine(HologramLine.text("<aqua>Page 2"));
```
...
### Hologram API Methods

```java
// Show / hide for a specific player
...
hologram.setFixedYaw(180f);         // Face North
hologram.setDoubleSided(true);

// Text settings
hologram.setTextAlignment(TextAlignment.LEFT);
hologram.setBackgroundAlpha(128);   // Semi-transparent background

// Visibility
hologram.setRenderDistance(64.0);   // Override default

// Auto-update intervals
hologram.setUpdateIntervalSeconds(5);   // Refresh content every 5 s
hologram.setPageIntervalSeconds(3);     // Auto-switch pages every 3 s
```
...
public class MyListener implements Listener {
    ...
    public void onHologramClick(HologramClickEvent event) {
        ...
        int lineIndex   = event.getLineIndex();  // 0-based

        player.sendMessage("You clicked line " + lineIndex + " of " + holo.getName());

        // Optionally cancel default click actions
...
public void onHologramCreate(HologramCreateEvent event) {
    // Access the newly registered hologram
    Hologram holo = event.getHologram();
}
```
...
### Q: The plugin doesn't load / I see an error on startup

**Check:**
1. Your server is running **Paper 1.20.5 or newer** (Spigot is not supported — `TEXT_DISPLAY` entities require Paper).
2. You are running **Java 21 or newer** (`java -version`).
3. The JAR is not corrupted — re-download if necessary.

---

### Q: Holograms are not visible

1. Make sure the hologram is **enabled**: `/kh info <name>` → check `Enabled: true`.
2. Check the **render distance**: use `/kh info <name>` and compare to your position.
3. If `fov-culling: true`, make sure you are **facing** the hologram.
4. Run `/kh reload` after editing the config.

---

### Q: PlaceholderAPI placeholders show as literal `%text%`

1. Verify PlaceholderAPI is installed and enabled: `/plugins`.
2. Install the required PAPI expansion (e.g. `/papi ecloud download Player` then `/papi reload`).
3. Make sure `placeholders.enabled: true` in `config.yml`.
4. The line must contain `%` — only then does PlaceholderAPI get invoked.

---

### Q: Legacy color codes (`&6`, `&#ff5500`) don't render

Change the config setting:
```yaml
text-format: legacy
```
Then run `/kh reload`. Remember: existing holograms with MiniMessage tags (like `<gold>`) will display as plain text after this change.

---

### Q: How do I make a hologram that shows on a wall?

Use `type: FIXED` and set an appropriate `fixed-yaw`:
```
/kh edit <name>
```
In the GUI, set **Type** to `FIXED` and adjust **Direction Angle** so the face points outward from the wall. Enable **Double-Sided** if you want it readable from both sides.

---

### Q: How do I automatically cycle through pages?

In the GUI (**Page Cycle Interval**) or directly in the YAML file:
```yaml
page-interval: 5   # switch pages every 5 seconds
```

---

### Q: Can I import holograms from HolographicDisplays / DecentHolograms?

Not automatically. You can manually create hologram files under `plugins/KapyHolograms/holograms/` following the format in [§10](#10-hologram-file-format).

---

### Q: How do I change the language for individual players?

Per-player language selection is not supported. The language is server-wide, configured via `language` in `config.yml`.

---

### Q: The server lags when there are many online players

Tune the performance settings in `config.yml`:
- Enable **FOV culling** (`fov-culling: true`) to skip invisible holograms.
- Increase `visibility-throttle-ticks` (e.g. `5`) to spread visibility checks.
- Enable **LOD** and increase `far-update-ticks` (e.g. `100`) to update distant holograms less often.
- Reduce `default-render-distance` so fewer holograms are loaded per player.
